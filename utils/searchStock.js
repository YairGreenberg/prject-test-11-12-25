...
{
    let listAbovePrice = 
    stockMarket.stocks.filter(object => object.currentPrice > price)
...
export function searchByName(stockMarket,nameStock)
{
    try{
    let listSameName =
    stockMarket.stocks.filter(object => object.name === nameStock)
    return listSameName
    }catch(error){
...
export function searchById(stockMarket,idStock)
{
    try{
    let listSameId =
    stockMarket.stocks.filter(object => object.id === idStock)
    return listSameId
    }catch(error){
        console.error("No matches found.");
        return [];
